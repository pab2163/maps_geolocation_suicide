---
title: "Data Cleaning & Aggregation (Single Home Location)"
...
# Read in EARS gps daily data, and separately coded entropy file
...
# Bring in covid data (for covariates)
```{r}
...
par(mfrow = c(2, 2)) 
mosaicplot(table(data_day_processed$status, data_day_processed$summer), shade = TRUE, las=2,
           main = "Status by vacation")
chi <- chisq.test(table(data_day_processed$status, data_day_processed$summer))
pairwiseNominalIndependence(table(data_day_processed$summer,data_day_processed$status), fisher = FALSE, method = 'fdr')


mosaicplot(table(data_day_processed$status, data_day_processed$week), shade = TRUE, las=2,
           main = "Status by vacation")
chisq.test(table(data_day_processed$status, data_day_processed$week))
pairwiseNominalIndependence(table(data_day_processed$week,data_day_processed$status), fisher = FALSE, method = 'fdr')
```
...
data_weekly = data_day_processed %>%
  ...
  #filter(!if_all(c(travel_km_mean), is.na)) %>% #daily_mean, n_char_mean
...
```{r}
write.csv(data_weekly_with_stb, file = 'cleaned_data/weekly_data_with_stb.csv', row.names = FALSE)
```
