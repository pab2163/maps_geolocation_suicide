...
tab_model(fit_ema_lead1week, fit_ema_lead2week, fit_ema_lead3week)
```
...
write.csv(within_participant_model_table, file = 'tables/within_participant_explanatory_model_table.csv', 
          row.names = FALSE)
```
...
cowplot::save_plot(fig1_grid, file = 'plots/fig1_grid.png', base_width = 10, base_height = 6)
```
...
ggsave(week_timeline_plot_ema, file = 'plots/supplement/week_timeline_plot_ema_idation.png', height = 4, width = 6)
...
cowplot::save_plot(timing_plot, file = 'plots/fig2_timing_plot_geo_events.png', base_height = 4, base_width = 10)
...
tidy_weeks %>% mutate(aOR = round(Estimate, 2), 
  ...
  write.csv(., file = 'tables/homestay_timing_table.csv', row.names = FALSE)
...
tidy_event_predict_geo %>%
  ...
  write.csv(file = 'tables/suicide_events_predict_homestay_table.csv', row.names = FALSE)
```
...
### Association between baseline variables & events


```{r}
fit_events_baseline = glmer(event_cat1  ~  SSI19_baseline_imp1month  +
                             week_num + (1|ID) , data = df,
              family = binomial(link='logit'), control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))

fit_si_baseline = glmer(ema_trigger  ~  SSI19_baseline_imp1month  +
                             week_num + (1|ID) , data = df,
              family = binomial(link='logit'), control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))


tab_model(fit_events_baseline, fit_si_baseline)
...
### Random slopes 
```{r}
fit_homestay_random_slopes = brms::brm(event_cat1_lead1week  ~  home_h_dev + entropy_dev + travel_km_dev + 
                                           home_h_idmean + entropy_idmean + travel_km_idmean  + 
                             week_num + (home_h_dev|ID) , data = df,
              family = bernoulli(link='logit'), iter = 2000, cores = 4, seed = 11291993)

summary(fit_homestay_random_slopes)
```

# Is there an interaction between the between-person and within-person effects?

```{r}
fit_within_between_homestay = glmer(event_cat1_lead1week  ~  home_h_dev*home_h_idmean + entropy_dev + travel_km_dev + 
                                            + entropy_idmean + travel_km_idmean  + 
                             week_num + (1|ID) , data = df,
              family = binomial(link='logit'), control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))

fit_within_between_entropy = glmer(event_cat1_lead1week  ~  home_h_dev + home_h_idmean + entropy_dev*entropy_idmean +
                                     travel_km_dev  +  travel_km_idmean + 
                             week_num + (1|ID) , data = df,
              family = binomial(link='logit'), control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))

fit_within_between_traveldist = glmer(event_cat1_lead1week  ~  home_h_dev + home_h_idmean +
                                        entropy_dev + travel_km_dev*travel_km_idmean + 
                                            + entropy_idmean + 
                             week_num + (1|ID) , data = df,
              family = binomial(link='logit'), control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e5)))

tab_model(fit_within_between_homestay, fit_within_between_entropy, fit_within_between_traveldist, transform = NULL)
```